...
[Preact](https://preactjs.com/) is a smaller, and faster alternative UI library to [React](https://reactjs.org/) with the same API.
...
First, set up a Preact project using the [official Preact CLI](https://github.com/developit/preact-cli). The CLI tool scaffolds a Preact PWA application for you quickly, under a minute.

If you don't have the Preact CLI installed globally, use the following command to install it globally:

<TerminalInput>yarn global add preact-cli</TerminalInput>
<Caption>Installing Preact CLI global to the system user with <GenericLink href="https://yarnpkg.com">Yarn</GenericLink>.</Caption>
...
Once the app is deployed, you will receive a deployment URL similar to the following: https://my-preact-project-ro3oxryae.now.sh/

You can see that our routes were successful by going to the generated `/profile` route directly: https://my-vue-project-ro3oxryae.now.sh/profile
...
```json
{
  ...
  "routes": [
    ...
    { "src": "/(.*)\\.(.*)\\.(js|css|json)$", "headers": {"cache-control": "public,max-age=31536000,immutable"}, "dest": "/$1.$2.$3" },
