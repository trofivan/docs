...
Now for GitLab is an app for GitLab that automatically deploys and alias to production for GitLab projects with Now.

The Now for GitLab integration features:
...
## Connecting the App to Your GitLab Account or Organization

You can simply connect the Now for GitLab app by visiting your account or team settings page. Then click the "Connect with GitLab" button.
...
Then you need to authorize the application on GitLab.
...
But you can always configure `now.json` to customize your app with different runtimes thanks to our v2 platform.

## A Deployment for each Push

Now for GitLab will **deploy every push by default**. This includes pushes to any branch and any pull requests made from those branches. This allows those working within the repository to preview changes made before the changes are pushed to production.
...
You may want to use different workflows and use different APIs based on Git information.In order to support that, Now will deploy your app with following built-in environment variables.
