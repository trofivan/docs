...
The `@now/wordpress` Builder takes in a `wp-config.php` file as an entrypoint, installs [WordPress](https://wordpress.org/download/), and outputs the result as a [Lambda](/docs/v2/deployments/concepts/lambdas).

## When to Use It

This Builder is the recommended way to quickly create a WordPress site that you plan to deploy with ZEIT Now.

### How to Use It

Create these two files within a directory like so:
...
Define the `ca.pem` source file by copying the contents of your [SSL CA certificate](https://en.wikipedia.org/wiki/Certificate_authority) from your MySQL cloud hosting provider to the file.

<Note>
  Ensure you pick a database location that matches a Now{' '}
...
<Caption>
  An example <InlineCode>wp-config.php</InlineCode> file defining the{' '}
...
Finally, define a `now.json` like so:
...
<Caption>
  An example <InlineCode>now.json</InlineCode> file defining the{' '}
...
>
  Read more on how to use the <InlineCode>now.json</InlineCode> file.
</Card>

At this point you are ready to make a deployment, this can be done by using the `now` command:

<TerminalInput>now</TerminalInput>

After running `now`, you'll receive a URL like this for you to share: https://wordpress-8yvrab9k2.now.sh/

### Adding Themes and Plugins

You can add themes and plugins to your WordPress site on ZEIT Now by following the instructions below:

#### Themes

- Download the theme of your choice from the [WordPress Theme Directory](https://wordpress.org/themes/).
- Unzip the zip file contents.
- Create a `wp-content/themes` directory in your project root and add the theme folder to it.
- Run the `now` command.

<TerminalInput>now</TerminalInput>

- Once your app has been deployed, navigate to the `/wp-admin` route of your URL and log in to your dashboard. Within the dashboard, go to **Appearance > Themes** and activate your theme.

#### Plugins

- Download the plugin you want from the [WordPress Plugins Directory](https://wordpress.org/plugins/).
- Unzip the zip file contents.
- Create a `wp-content/plugins` directory in your project root and add the plugin folder to it.
- Run the `now` command.

<TerminalInput>now</TerminalInput>

- Once your app has been deployed, navigate to the `/wp-admin` route of your URL and log in to your dashboard. Within the dashboard, go to **Plugins > Installed Plugins** and activate your plugin.

## Technical Details

### Entrypoint

The entrypoint must always be a `wp-config.php` source file.
