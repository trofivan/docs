...
The `@now/wordpress` builder takes in a `wp-config.php` file as an entry point, installs wordpress, and outputs it as a [Lambda](/docs/v2/deployments/concepts/lambdas).
...
Define the `ca.pem` source file by copying the contents of your **SSL CA certificate** from your MySQL hosting service to the file.
