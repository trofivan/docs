...
The `@now/wordpress` builder takes in a `wp-config.php` file as an entry point, builds its dependencies, and outputs it as a [Lambda](/docs/v2/deployments/concepts/lambdas).
...
- `ca.pem` - A source file that contains the private key for securely connecting to a hosted MySQL database.
- `wp-config.php` - A configuration file.

Define the `ca.pem` source file by copying the contents of your `SSL CA cert` from your MySQL hosting service to the file.
...
And finally, define a `now.json` like:
...
The entrypoint file must be a `wp-config.php` source file.
...
The entrypoint of this Builder is always a `wp-config.php` file.
